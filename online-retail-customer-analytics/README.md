...
Champions and Loyals together — 46% of the base — are holding up the entire revenue base, generating over 5.6x the value of all other segments combined. The bottom 30% of customers contribute just 2.3% of revenue.

![RFM Segment Concentration](screenshots/rfm_segment_concentration.png)

→ **Size the risk before acting on growth.** Losing 10% of Champions represents a £1.17M revenue hit — larger than the entire Potential and Hibernating segment revenue combined. Retention investment for Champions has a clear ceiling to work from.

→ **Upgrade Loyals before chasing new customers.** If 20% of Loyals reach Champion-level behaviour, that's £1.92M in incremental revenue from the existing base with zero acquisition cost — a 69% revenue lift for the entire Loyal segment.
...
At-Risk customers have a median order value of £300 vs £258 for Loyals — meaning each At-Risk transaction is worth more than a Loyal one. The business is losing its higher-value buyers while retaining its lower-value ones.
